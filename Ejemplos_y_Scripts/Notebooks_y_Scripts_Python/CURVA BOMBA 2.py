...
class PumpCurveApp:
    ...
    def update_plots(self):
        ...
        if q_bep_new is not None:
            self.ax1.plot(q_bep_new, h_bep_new, 'ms', markersize=8, label=f'BEP Nueva: Q={q_bep_new:.1f}, H={h_bep_new:.1f}') # Cambiado a magenta square
            self.ax1.annotate(f'BEP Nueva\nQ={q_bep_new:.1f}\nH={h_bep_new:.1f}', 
                              xy=(q_bep_new, h_bep_new), xytext=(q_bep_new - 150, h_bep_new - 15), # Ajustar posición
        ...
        self.ax2.plot(q_new, eff_new, 'co-', label=f'Nueva: D2={d2}", N2={n2} RPM') # Cambiado a cyan
        ...
        if q_bep_new is not None:
            self.ax2.plot(q_bep_new, eff_bep_new, 'ms', markersize=8, label=f'BEP Nueva: Q={q_bep_new:.1f}, Eff={eff_bep_new:.1f}%') # Cambiado a magenta square
            self.ax2.annotate(f'BEP Nueva\nQ={q_bep_new:.1f}\nEff={eff_bep_new:.1f}%',
                              xy=(q_bep_new, eff_bep_new), xytext=(q_bep_new -150, eff_bep_new + 5), # Ajustar posición
                              arrowprops=dict(facecolor='cyan', shrink=0.05, width=1, headwidth=5,connectionstyle="arc3,rad=-.2"),
                              bbox=dict(boxstyle="round,pad=0.3", fc="lightcyan", ec="cyan", lw=0.5, alpha=0.8))
